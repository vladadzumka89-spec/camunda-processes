...
- `POOL_X=105, POOL_Y=90`, `LANE_LABEL_WIDTH=30`, `LANE_X=135`
- `H_SPACING=80` — мінімальний горизонтальний зазор між правим краєм і лівим наступного
...
2. **L-форма (та сама lane, різний Y):** 3-4 waypoints — горизонтально, вертикально, горизонтально
3. **Z-форма (крос-lane):** 4 waypoints — горизонтально, вертикально, горизонтально
4. **Зворотній потік:** 6 waypoints — обхід через corridor вище pool (`y = POOL_Y - 10`)
5. **Boundary event:** 2-3 waypoints — вертикально вниз, потім горизонтально до target
