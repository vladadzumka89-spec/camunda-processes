"""Webhook server for CI/CD worker — GitHub/Odoo events to Camunda Zeebe.
...
class WebhookServer:
    ...
    async def _route_push_event(self, payload: dict) -> web.Response:
        ...
        # Extract PR info from merge commit message
        head_commit = payload.get('head_commit', {})
        head_message = head_commit.get('message', '')
        pr_match = _MERGE_PR_RE.search(head_message)

        pr_number = int(pr_match.group(1)) if pr_match else None
        # PR title is the second line of merge commit message
        pr_title = ''
        if pr_match and '\n' in head_message:
            pr_title = head_message.split('\n', 2)[1].strip()
