...
def _redact_pat(text: str, pat: str) -> str:
    """Remove PAT from error messages to prevent credential leakage."""
...
def register_sync_handlers(
    ...
    async def _ws_run(server, cmd: str, workspace: str = "", **kwargs):
        ...
        ws = workspace
        if not ws:
            raise ConfigError("workspace path is required for _ws_run")
        ...
        try:
            ...
            try:
                await ssh.run(
                    ...
                    f"{_git_auth_url(deploy_pat, repo)} {workspace_dir}",
                    check=True,
                    timeout=300,
                )
            except Exception as e:
    ...
    async def sync_modules(
        ...
        **kwargs: Any,
        ...
        if not community_dir or not enterprise_dir or not workspace_dir:
            raise ConfigError("community_dir, enterprise_dir, workspace_dir are required")
        ...
        if modules:
            ...
            if synced == 0:
                raise SyncError("No valid modules found in upstream")
    ...
    async def diff_report(
        ...
        **kwargs: Any,
        ...
        if not workspace_dir:
            raise ConfigError("workspace_dir is required")
        ws = workspace_dir
    ...
    async def merge_to_staging(
        ...
        **kwargs: Any,
        ...
        """Hard-reset staging to sync branch and force-push.

        Works in the existing repo on the server (no clone needed).
        Fetches the sync branch, resets staging to it, and force-pushes.
        """
        ...
        try:
            await ssh.run(server, reset_cmd, check=True, timeout=120)
        except Exception as e:
            raise SyncError(_redact_pat(str(e), deploy_pat)) from None
        logger.info("Reset staging to %s on %s", sync_branch, server.host)
